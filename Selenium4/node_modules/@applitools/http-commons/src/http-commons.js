...
async function throwErrorFromBadStatus(url, response) {
  const err = new Error(
    `Response ${response.status} returned from ${url}, body: ${await response.text()}`,
  )
 ...
 * @param {{alternativeFetch?: Function}} [options]
...
async function fetchBySubstituteDnsNameWithIp(fetch, url, fetchOptions, parentPerformance) {
  ...
  if (!ENABLE_DNS_CACHING) {
    return await timeItAsync('headers', () => fetch(url, fetchOptionsWithAgent))
  }
